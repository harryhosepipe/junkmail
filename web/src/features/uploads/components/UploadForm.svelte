<script>
  ...
  import { createAuthSessionStore } from "../../auth/store";
  import { createImagesApi } from "../../images/api";
  ...
  let uploadsApiBaseUrl = "";
  let uploadsApi = createUploadsApi(apiBaseUrl);
  ...
  const fetchImageDetail = async (imageId) => {
    try {
      return await imagesApi.getImageById(imageId);
    } catch {
      return null;
    }
  };
  ...
  const pollImage = async (imageId, meta) => {
    ...
    const tick = async () => {
      attempts += 1;
      try {
        const data = await imagesApi.getImageById(imageId);
  ...
  const pollUploadStatus = async (uploadId, imageId, meta) => {
    ...
    const tick = async () => {
      attempts += 1;
      try {
        const payload = await uploadsApi.getUploadStatus(uploadId);
  ...
  const uploadLegacy = async ({ currentDescription }) => {
    ...
    let data = null;
    try {
      data = await imagesApi.uploadLegacyImage(formData);
    } catch (error) {
      ...
      if (
        ...
        "duplicateType" in payload &&
        payload.duplicate &&
        payload.duplicateType === "near"
      ) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        ...
        setStatus(
          error instanceof Error && error.message ? error.message : "Could not initialize upload.",
          "error",
        );
        return;
      }
      ...
      try {
        await uploadsApi.completeUpload(completeForm);
      } catch (error) {
        if (error instanceof ApiError && (error.status === 401 || error.status === 403)) {
          setStatus("Session expired. Request a new link.", "error");
          authState = "guest";
  ...
  $: if (apiBaseUrl !== uploadsApiBaseUrl) {
    uploadsApi = createUploadsApi(apiBaseUrl);
    uploadsApiBaseUrl = apiBaseUrl;
  }
