<script>
  ...
  import { createVotingStore } from "../store";
  ...
  let votingStoreBaseUrl = "";
  let votingStore = createVotingStore(apiBaseUrl);
  ...
  const loadMatchup = async ({ keepCurrent = false } = {}) => {
    ...
    errorMessage = "";
    try {
      matchup = await votingStore.requestMatchup();
      state = "ready";
      votingStore.prefetchNextMatchup();
      return true;
    } catch (err) {
  ...
  const submitVote = async (winnerId) => {
    ...
    const votePromise = votingStore.sendVote({
      imageAId,
      imageBId,
      winnerId,
    ...
    const nextMatchup = await votingStore.consumePrefetchedMatchup();
    try {
      await votePromise;
    } catch (err) {
  ...
  $: if (apiBaseUrl !== votingStoreBaseUrl) {
    votingStore = createVotingStore(apiBaseUrl);
    votingStoreBaseUrl = apiBaseUrl;
  }
